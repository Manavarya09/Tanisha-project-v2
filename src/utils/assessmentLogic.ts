...
export function calculateAssessmentResults(data: AssessmentData): AssessmentResults {
  ...
  // Use CSV-based questions for scoring
  const region = data.region || 'Global';
  const pillarQuestions = parseAirtableCSVByRegion(airtableCsv, region);
  const fallback = parseAssessmentCSVUnique(csvText);
  const allQuestions = { ...fallback, ...pillarQuestions };

  console.log('Available pillars:', Object.keys(allQuestions));

  // Calculate individual pillar scores using CSV-based questions
  const strategyScore = calculatePillarScoreFromCSV(allQuestions['Strategy & Leadership'] || {}, data.responses);
  const cultureScore = calculatePillarScoreFromCSV(allQuestions['AI Organization & Culture'] || {}, data.responses);
  const businessScore = calculatePillarScoreFromCSV(allQuestions['Business Readiness'] || {}, data.responses);
  const dataScore = calculatePillarScoreFromCSV(allQuestions['Data Readiness'] || {}, data.responses);
  const infrastructureScore = calculatePillarScoreFromCSV(allQuestions['Infrastructure Readiness'] || {}, data.responses);
  const peopleScore = calculatePillarScoreFromCSV(allQuestions['People & Skills'] || {}, data.responses);
  const governanceScore = calculatePillarScoreFromCSV(allQuestions['AI Governance & Ethics'] || {}, data.responses);
  ...
  const strategyCount = Object.values(allQuestions['Strategy & Leadership'] || {}).flat().length;
  const cultureCount = Object.values(allQuestions['AI Organization & Culture'] || {}).flat().length;
  const businessCount = Object.values(allQuestions['Business Readiness'] || {}).flat().length;
  const dataCount = Object.values(allQuestions['Data Readiness'] || {}).flat().length;
  const infrastructureCount = Object.values(allQuestions['Infrastructure Readiness'] || {}).flat().length;
  const peopleCount = Object.values(allQuestions['People & Skills'] || {}).flat().length;
  const governanceCount = Object.values(allQuestions['AI Governance & Ethics'] || {}).flat().length;

  const pillarScores: PillarScore[] = [
    {
      name: 'Strategy & Leadership',
      score: Math.round((strategyScore / 100) * strategyCount * 5),
      maxScore: strategyCount * 5,
    ...
    },
    {
      ...
      score: Math.round((cultureScore / 100) * cultureCount * 5),
      maxScore: cultureCount * 5,
    ...
    },
    {
      ...
      score: Math.round((businessScore / 100) * businessCount * 5),
      maxScore: businessCount * 5,
    ...
    {
      name: 'Data Readiness',
      score: Math.round((dataScore / 100) * dataCount * 5),
      maxScore: dataCount * 5,
    ...
    },
    {
      ...
      score: Math.round((infrastructureScore / 100) * infrastructureCount * 5),
      maxScore: infrastructureCount * 5,
    ...
    {
      name: 'People & Skills',
      score: Math.round((peopleScore / 100) * peopleCount * 5),
      maxScore: peopleCount * 5,
    ...
    },
    {
      ...
      score: Math.round((governanceScore / 100) * governanceCount * 5),
      maxScore: governanceCount * 5,
  ...
  const totalQuestions = strategyCount + cultureCount + businessCount + dataCount + 
    infrastructureCount + peopleCount + governanceCount;
...
function calculatePillarScoreFromCSV(pillarQuestions: { [subcategory: string]: UniqueCSVQuestion[] }, responses: Record<string, number>): number {
  const allQuestions = Object.values(pillarQuestions).flat();
  if (allQuestions.length === 0) return 0;
  
  const scores = allQuestions.map(q => responses[q.question] || 1);
  const average = scores.reduce((sum, score) => sum + score, 0) / scores.length;
  return Math.round((average / 5) * 100);
}
