...
export default function DetailedReport() {
    ...
    try {
      ...
      if (button) {
        ...
        try {
          ...
          // Configure html2canvas for better quality with specific settings for PDF
          ...
          const imgWidth = pdfWidth - 40; // Increased margins for better readability
          const imgHeight = (canvas.height * imgWidth) / canvas.width;
          
          // Calculate how many pages we need with better margins
          const pageHeight = pdfHeight - 40; // Increased margins
          const totalPages = Math.ceil(imgHeight / pageHeight);

          // Add content to PDF with proper page breaks and margins
          for (let i = 0; i < totalPages; i++) {
            if (i > 0) {
              pdf.addPage();
            }
            // Calculate the portion of the image to draw for this page
            // Use pageHeight for each page, and offset the source image accordingly
            const srcY = i * pageHeight * (canvas.height / imgHeight);
            ...
            if (ctx) {
              ctx.drawImage(
                canvas,
                0, srcY, canvas.width, srcHeight, // source
                0, 0, canvas.width, srcHeight // destination
              );
            }
            const pageImgData = pageCanvas.toDataURL('image/png', 0.95);
            pdf.addImage(pageImgData, 'PNG', 20, 20, imgWidth, pageHeight, undefined, 'FAST');
          }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div id="pdf-report-content" className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10 py-10" style={{ 
              ...
              <Card key={index} style={{ 
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <p className="text-gray-700 text-base leading-relaxed mb-4" style={{ 
                      marginBottom: '20px',
                      lineHeight: '1.6',
                      fontSize: '16px'
                    }}>
                    ...
                    <div className="grid md:grid-cols-3 gap-8 mt-8" style={{ 
                      pageBreakInside: 'avoid',
                      breakInside: 'avoid'
                    }}>
                      <div className="text-center p-6 bg-white rounded-lg border-2 border-gray-300 shadow-sm" style={{ 
                        minHeight: '120px',
                        display: 'flex',
                        ...
                        <div className="text-3xl font-bold text-gray-900 mb-2">{pillar.score}</div>
                        <div className="text-sm font-semibold text-gray-700">Raw Score</div>
                      </div>
                      <div className="text-center p-6 bg-white rounded-lg border-2 border-gray-300 shadow-sm" style={{ 
                        minHeight: '120px',
                        display: 'flex',
                        ...
                        <div className="text-3xl font-bold text-gray-900 mb-2">{pillar.maxScore}</div>
                        <div className="text-sm font-semibold text-gray-700">Max Possible</div>
                      </div>
                      <div className="text-center p-6 bg-white rounded-lg border-2 border-gray-300 shadow-sm" style={{ 
                        minHeight: '120px',
                        display: 'flex',
                        ...
                        <div className="text-3xl font-bold text-gray-900 mb-2">{pillar.level}</div>
                        <div className="text-sm font-semibold text-gray-700">Performance Level</div>
                      </div>
                    </div>
                  </div>
                </CardContent>
